...
import LightXML: content, get_elements_by_tagname, parse_file, find_element, root

include(joinpath(@__DIR__,"..","..","..","src","common.jl"))

labels_dir = joinpath(datasets_dir,"voc","VOCdevkit","VOC2007","Annotations")
...
function collectLabelImagePairs(labels_dir::String, images_dir::String)
    ...
    for (root, dirs, files) in walkdir(labels_dir)
        label_filepaths = joinpath.(labels_dir, filter(x->occursin(".xml",x),files))
        image_filepaths = joinpath.(images_dir, map(x->string(first(split(x,".xml")),".jpg"),files))
...
function loadLabel(label_file::String)
    ...
    for i in 1:length(ces)
        ...
        if difficult == 0 #TODO allow different difficulties
            ...
            xmin = parse(Int32,content(find_element(bbox_loc,"xmin")))/image_width
            xmax = parse(Int32,content(find_element(bbox_loc,"xmax")))/image_width
            ymin = parse(Int32,content(find_element(bbox_loc,"ymin")))/image_height
            ymax = parse(Int32,content(find_element(bbox_loc,"ymax")))/image_height
            push!(YLs, Label(x = xmin, y = ymin, w = xmax-xmin, h = ymax-ymin,
