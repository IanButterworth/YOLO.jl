...
"""
function resizePadImage(img::Array{T}, model::Yolo) where {T<:ImageCore.Colorant}
    if size(img) == (model.cfg[:width], model.cfg[:height])
        return permutedims(Float32.(channelview(img)), [3,2,1])[:,:,1:3]
    end
    img_size = size(img)
    model_size = (model.cfg[:width], model.cfg[:height], model.cfg[:channels])
    target_img_size = sizethatfits(img_size, model_size)
    kern = resizekern(img_size, target_img_size)
    return resizePadImage(img, model_size, kern)
end
function resizePadImage(img::Array{T}, target_img_size::Tuple{Int, Int, Int}, kern) where {T<:ImageCore.Colorant}
    target_img = zeros(Float32, target_img_size)
